...
# initial catalog
init_cat=join(os.environ[env], "catalogs", "zcat.deep2.dr4.v4.LFcatalogTC.Planck15.fits")
...
chi2 = n.ones((len(orig_table['MASK']),9))*dV
ndof = n.ones((len(orig_table['MASK']),9))*dV
absolute_magnitudes = n.ones((len(orig_table['MASK']),5))*dV
absolute_magnitudes_err = n.ones((len(orig_table['MASK']),5))*dV

def plot_spec_spFly(mask, objno, id_spec):
	"""
	create a plot per spectrum and computes the chi2 and number of degrees of freedom for each model.
	
	"""
	spFly_file = os.path.join(os.environ[env], 'stellarpop', 'spFly-deep2-'+mask+'-'+objno+".fits")
	if os.path.isfile(spFly_file) :
		print (spFly_file)
		# defines the output paths
		im_file = 'spFly-'+mask+'-'+objno+'.png'
		...
		if len(cvm)>=1:
			...
			spec_file = join(os.environ[env], 'spec', 'deep2-'+mask+'-'+objno +".fits")
			obs = fits.open(spec_file)
			# interpolates the observations and the errors
			wl_data = obs[1].data['wavelength']/(1+obs[0].header['redshift'])
			fl_data = obs[1].data['flux']
			err_data = obs[1].data['flux_error']
			ok_data = (obs[1].data['flux_error']>0)
			...
			distMod = cosmo.distmod( obs[0].header['redshift'] ).value 
			...
			p.title("mask=" + mask + ", objno=" + objno + ", z=" + str(n.round(obs[0].header['redshift'],3)))
...
for id_spec in range(len(orig_table['MASK'])):
	mask     	= str(orig_table['MASK'][id_spec])
	objno 		= str(orig_table['OBJNO'][id_spec]) 
	print mask, objno
	plot_spec_spFly(mask, objno, id_spec)
...
path_2_out_file = join( out_dir, "zcat.deep2.dr4.v4.LFcatalogTC.Planck15.spm.fits")
...
table_all = n.ones(( len(orig_table['OBJNO']), 513)) * dV
headers = ""
for index, (objno, mask) in enumerate(zip(orig_table['OBJNO'], orig_table['MASK'])):
	fitFile = join( os.environ[env], dir, "spFly-deep2-"+str(mask)+"-"+str(objno)+".fits")
